...
trait Solver extends GameDef {
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    if (initial.isEmpty) Stream.empty
      ...
      // each (Block, List[Move]) is containing current block location and until history(Move).
      // e.g. some Block, List(left, right, up), some Block, List(left, right, down, up)
  ...
  lazy val solution: List[Move] = pathsToGoal match {
    ...
    // if (pathsToGoal.isEmpty) List() else pathsToGoal.minBy(_._2.size)._2
  }
}
